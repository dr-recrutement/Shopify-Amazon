...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  const rows: Array<{ dns_status: string; tenants?: { owner_id: string } }> = await domainRes.json();
  const row = rows?.[0];
  ...
  const cfData: { success: boolean; result?: { status?: string; certificate_authority?: unknown } } = await cfRes.json();
