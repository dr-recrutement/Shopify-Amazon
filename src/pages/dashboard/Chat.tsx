      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 h-[600px]">
        <Card className="overflow-y-auto scrollbar-thin">
          {convs.map(c => {
            ...
            return (
              <button key={c.id} onClick={() => setActive(c.id)} className={`w-full text-left p-3 border-b border-gray-50 hover:bg-gray-50 ${active === c.id ? 'bg-orange-50' : ''}`}>
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-full bg-orange-100 flex items-center justify-center text-sm font-semibold text-orange-700">{c.name.charAt(0)}</div>
                  ...
                  {c.unread > 0 && <span className="w-5 h-5 bg-orange-600 text-white text-xs rounded-full flex items-center justify-center">{c.unread}</span>}
                </div>
              </button>
            );
          })}
        ...
        <Card className="lg:col-span-2 flex flex-col">
          <div className="p-4 border-b border-gray-100 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full bg-orange-100 flex items-center justify-center text-sm font-semibold text-orange-700">A</div>
              <div>
          ...
          <div className="flex-1 p-4 overflow-y-auto scrollbar-thin space-y-3">
            ...
            <div className="flex justify-end"><div className="bg-orange-600 text-white rounded-2xl rounded-tr-sm px-3 py-2 text-sm max-w-xs">Bonjour ! Oui, disponible en S, M, L.</div></div>
          ...
          <div className="p-3 border-t border-gray-100 flex items-center gap-2">
            ...
            <input placeholder="Répondre..." className="flex-1 px-3 py-2 bg-gray-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" />
