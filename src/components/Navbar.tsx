      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center gap-1">
            <Link to="/marketplace" className={`px-3 py-2 text-sm font-medium ${linkColor} hover:text-orange-600 transition-colors`}>{t('nav.marketplace')}</Link>
            <Link to="/pricing" className={`px-3 py-2 text-sm font-medium ${linkColor} hover:text-orange-600 transition-colors`}>{t('nav.pricing')}</Link>

            <div className="relative" onMouseEnter={() => setDropdown('features')} onMouseLeave={() => setDropdown(null)}>
              <button className={`flex items-center gap-1 px-3 py-2 text-sm font-medium ${linkColor} hover:text-orange-600 transition-colors`}>
            ...
            <Link to="/about" className={`px-3 py-2 text-sm font-medium ${linkColor} hover:text-orange-600 transition-colors`}>{t('nav.about')}</Link>
          </nav>
              ...
              {langOpen && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-1 w-32 bg-white rounded-lg border border-gray-100 shadow-lg py-1 z-20">
                    <button onClick={() => { setLang('fr'); setLangOpen(false); }} className={`w-full text-left px-3 py-1.5 text-sm hover:bg-gray-50 ${lang === 'fr' ? 'text-orange-600 font-medium' : 'text-gray-700'}`}>Français</button>
                    <button onClick={() => { setLang('en'); setLangOpen(false); }} className={`w-full text-left px-3 py-1.5 text-sm hover:bg-gray-50 ${lang === 'en' ? 'text-orange-600 font-medium' : 'text-gray-700'}`}>English</button>
                  </div>
                </>
              )}
            </div>
            <Link to="/login" className="text-sm font-medium text-gray-700 hover:text-orange-600 px-3 py-2 rounded-lg transition-colors">{t('nav.login')}</Link>
            <Link to="/register" className="text-sm font-semibold text-white bg-orange-600 hover:bg-orange-700 px-4 py-2 rounded-lg transition-all hover:scale-[1.02] active:scale-95">{t('nav.register')}</Link>
          </div>
      ...
      {open && (
        <div className="md:hidden bg-white border-t border-gray-100 px-4 py-4 space-y-1">
          ...
          <div className="flex gap-2 pt-3 border-t border-gray-100">
            ...
            <Link to="/register" className="flex-1 text-center text-sm font-semibold text-white bg-orange-600 px-4 py-2 rounded-lg">{t('nav.register')}</Link>
          </div>
        </div>
      )}
    </header>
