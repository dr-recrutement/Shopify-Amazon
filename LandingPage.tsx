...
import {
  ...
  ChevronLeft, ChevronRight, X, Layers, Code2, Building2
} from 'lucide-react';
...
const comparisonRows = [
  { label: 'Commission sur les ventes', sellia: '0%, à vie', others: "1% à 3% par transaction" },
  { label: 'Mobile Money natif', sellia: true, others: false },
  { label: 'Paiements internationaux', sellia: true, others: true },
  { label: 'IA incluse (descriptions, visuels, chatbot)', sellia: true, others: 'Souvent en add-on payant' },
  { label: 'Domaine personnalisé', sellia: 'Inclus dès le plan Starter', others: 'Souvent payant en supplément' },
  { label: 'Boîte de réception unifiée (WhatsApp, Messenger)', sellia: true, others: false },
  { label: "Mise en ligne d'une boutique", sellia: '< 10 minutes', others: 'Souvent plusieurs heures' },
];

const scaleCapabilities = [
  { icon: Layers, title: 'Architecture composable', desc: "API-first, thèmes découplés du moteur — étendez ou remplacez chaque brique (catalogue, checkout, CMS) sans réécrire la plateforme." },
  { icon: Code2, title: 'API développeur ouverte', desc: "Endpoints REST documentés pour catalogue, commandes, clients et webhooks — intégrez Sellia à votre ERP, ADS manager ou outil interne." },
  { icon: Building2, title: 'Multi-tenant à isolation stricte', desc: "Chaque boutique tourne sur des politiques RLS dédiées en base de données — vos données restent cloisonnées, même à grande échelle." },
  { icon: Shield, title: 'Conçu pour la charge', desc: "CDN, edge functions et cache adaptés aux pics de trafic (lancements, soldes, campagnes virales) sans configuration supplémentaire." },
];
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Composable / scale architecture — BigCommerce-style enterprise-readiness section */}
      <section className="py-24 bg-gray-950 text-white relative overflow-hidden">
        <div className="absolute inset-0 opacity-20 pointer-events-none">
          <div className="absolute top-0 left-1/3 w-96 h-96 bg-brand-500 rounded-full mix-blend-screen filter blur-3xl" />
        </div>
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Section className="text-center max-w-2xl mx-auto mb-16">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-white/10 backdrop-blur-md border border-white/20 rounded-full text-sm font-semibold mb-6">
              <Layers size={14} className="text-brand-400" /> Prêt pour l'échelle
            </div>
            <h2 className="text-3xl sm:text-4xl font-bold tracking-tight">Une plateforme pensée pour grandir sans limite</h2>
            <p className="mt-4 text-gray-300">
              Des premières ventes aux marques qui traitent des milliers de commandes par jour, l'architecture de Sellia
              suit votre croissance — sans migration forcée vers un autre outil.
            </p>
          </Section>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {scaleCapabilities.map((c, i) => (
              <Section key={c.title} delay={i * 80}>
                <div className="h-full p-6 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 transition-colors">
      ...
      {/* Comparison table — Sellia vs traditional solutions */}
      <section className="py-24 bg-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <Section className="text-center max-w-2xl mx-auto mb-12">
            <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 tracking-tight">Sellia face aux solutions traditionnelles</h2>
            <p className="mt-4 text-gray-600">Ce que change une plateforme conçue dès le départ pour le commerce mondial et le Mobile Money.</p>
          </Section>
          <Section>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    <th className="text-left font-semibold text-gray-500 px-6 py-4">Critère</th>
                    <th className="text-center font-bold text-brand-700 px-6 py-4">Sellia</th>
                    <th className="text-center font-semibold text-gray-500 px-6 py-4">Solutions traditionnelles</th>
                  </tr>
                </thead>
                <tbody>
                  {comparisonRows.map((row, i) => (
                    <tr key={row.label} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}>
                      ...
                      <td className="px-6 py-4 text-center">
                        {typeof row.sellia === 'boolean' ? (
                          row.sellia ? <CheckCircle2 size={18} className="text-brand-600 inline" /> : <X size={18} className="text-gray-300 inline" />
                        ) : (
                          <span className="font-semibold text-brand-700">{row.sellia}</span>
                        )}
                      </td>
                      <td className="px-6 py-4 text-center">
                        {typeof row.others === 'boolean' ? (
                          row.others ? <CheckCircle2 size={18} className="text-gray-400 inline" /> : <X size={18} className="text-gray-300 inline" />
                        ) : (
